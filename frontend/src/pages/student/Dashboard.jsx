...
const Dashboard = () => {
  ...
  const [data, setData] = useState(() => getCachedData('dashboard', user?.username));
  const [loading, setLoading] = useState(() => !getCachedData('dashboard', user?.username));
  ...
  const handleMarkAttendance = async (timetableId, status) => {
    if (isOffline) {
      setError('Cannot mark attendance while offline.');
      return;
    }
    setError('');
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-1.5">
                      {/* PRESENT Toggle */}
                      <button
                        onClick={() => !isOffline && cls?.timetable?.id && handleMarkAttendance(cls.timetable.id, 'PRESENT')}
                        disabled={isOffline}
                        className={`p-1.5 rounded-lg transition-all flex items-center justify-center border ${
                          isOffline ? 'opacity-40 cursor-not-allowed border-cp-border text-cp-text-secondary' :
                          markedPresent
                      ...
                      {/* ABSENT Toggle */}
                      <button
                        onClick={() => !isOffline && cls?.timetable?.id && handleMarkAttendance(cls.timetable.id, 'ABSENT')}
                        disabled={isOffline}
                        className={`p-1.5 rounded-lg transition-all flex items-center justify-center border ${
                          isOffline ? 'opacity-40 cursor-not-allowed border-cp-border text-cp-text-secondary' :
                          markedAbsent
